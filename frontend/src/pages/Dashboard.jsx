...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        ...
        // Update API key if backend returns a new one
        const newToken = res.data.uuidToken;
        if (uuidApiKey !== newToken) {
          localStorage.setItem("uuidApiKey", newToken);
        }
      } catch (error) {
        console.error("Token missing or invalid, redirecting to login:", error);
        localStorage.removeItem("uuidApiKey");
        navigate("/login");
      }
    };
  ...
  const confirmSignOut = async () => {
    ...
    if (!uuidApiKey) {
      console.error("API key missing. Cannot sign out.");
      navigate("/login");
      return;
    }
